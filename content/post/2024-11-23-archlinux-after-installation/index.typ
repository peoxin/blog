...
#show: post.with(
  ...
  pub-date: datetime(year: 2024, month: 11, day: 23),
  mod-date: datetime(year: 2024, month: 11, day: 23),
)
...
通过 #link("https://github.com/521xueweihan/GitHub520")[GitHub520] 项目，可以获得最新的 GitHub IP 地址，然后将其添加到 `/etc/hosts` 文件的末尾即可。
根据该项目的说明，通过 #link("https://raw.hellogithub.com/hosts")[https://raw.hellogithub.com/hosts] 网址，可以在无需访问 GitHub 的情况下，获得与项目中相同的 GitHub IP 地址。
...
所需的配置文件，可以从订阅链接转换获得，并放置到 `~/.config/clash` 目录下。相关的转换工具：#link("https://github.com/tindy2013/subconverter")[subconverter]、#link("https://github.com/stilleshan/subweb")[subweb]。
也可以使用其他客户端转换过的配置文件，通过 USB 等方式传输到本机上。

从 #link("https://github.com/Loyalsoldier/geoip")[geoip] 项目中，可以获得 `Country.mmdb` 文件，将其放置到 `~/.config/clash` 目录下。

对于 `clash` 来说，dashboard 不是必需的。
如果需要，可以使用公共的 dashboard，例如：#link("http://clash.razord.top")[http://clash.razord.top]、#link("http://yacd.haishan.me")[http://yacd.haishan.me]。
也可以使用本地部署的 dashboard，例如 #link("https://github.com/haishanh/yacd")[yacd]，根据项目的说明操作，部署本地服务。
...
其中，`secret` 是可选的密码。在启动 `clash` 后，通过 #link("http://127.0.0.1:9090/ui")[http://127.0.0.1:9090/ui] 地址，即可访问 dashboard。
...
通过设置环境变量的方法，可以设置系统代理。具体细节可以参考：#link("https://wiki.archlinux.org/title/Environment_variables")[Environment variables]、#link("https://wiki.archlinux.org/title/Proxy_server")[Proxy server]。
...
为了更方便地管理系统代理，在 #link("https://github.com/peoxin/dotfiles")[dotfiles] 中编写了相关脚本，可供参考。

==== 2.3.3 安装 `clash-verge-rev-bin`

`clash-verge-rev-bin` 软件包位于 AUR 中。因为此时无法直接访问 AUR，所以参考 #link("https://wiki.archlinux.org/title/Arch_User_Repository")[Arch User Repository]，可以采用以下方法：

- 在其他设备上下载该软件包的构建文件，然后通过 USB 等方式传输到本机上；
- 使用 GitHub 上的只读镜像 #link("https://github.com/archlinux/aur")[aur on GitHub]。使用以下命令获得软件包的构建文件，并进行构建和安装：
...
根据 #link("https://wiki.archlinux.org/title/AUR_helpers")[AUR helpers] 中的说明，有多种 AUR 助手可供选择。
下面以 `paru` 为例，演示安装方法：
...
如果使用 NVIDIA 显卡，参考 #link("https://wiki.archlinux.org/title/NVIDIA")[NVIDIA - ArchWiki] 和 #link("https://wiki.hyprland.org/Nvidia")[NVidia - Hyprland] 来安装驱动。对于 Turing 架构及更新的显卡，可以安装以下软件包：
