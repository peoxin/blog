...
#show: post.with(
  ...
  pub-date: datetime(year: 2023, month: 2, day: 9),
  mod-date: datetime(year: 2023, month: 2, day: 9),
)

= Arch Linux 安装流程

Arch Linux 的安装流程，主要参考 Arch Linux 官方文档 #link("https://wiki.archlinux.org/title/Installation_guide")[Installation guide - ArchWiki]。

== 1 安装前准备

=== 1.1 下载安装镜像

在 #link("https://mirrors.tuna.tsinghua.edu.cn/archlinux/iso/")[Arch Linux 清华源]，下载安装镜像 `archlinux-version-x86_64.iso`。
...
#quote(block: true)[
  *注意*：挂载硬盘分区时，必须依照挂载点的层次关系，按顺序挂载。即首先挂载 Root 分区，然后才能挂载 EFI 和 Home 分区。
]
