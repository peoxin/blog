...
#show: post.with(
  title: "Python 解决导入问题",
  pub-date: datetime(year: 2024, month: 11, day: 11),
  mod-date: datetime(year: 2024, month: 11, day: 11),
)
...
在 Python 的官方文档中，对于 #link("https://docs.python.org/3/reference/import.html")[导入系统 The import system] 进行了详细的介绍。
但是，官方文档中的介绍比较长，因此以下摘录其中的要点。

=== 1.1 #link("https://docs.python.org/3/reference/import.html#packages")[Module 和 Package]

#quote(block: true)[
  Python has only one type of #link("https://docs.python.org/3/glossary.html#term-module")[module] object, and all modules are of this type, regardless of whether the module is implemented in Python, C, or something else.
  To help organize modules and provide a naming hierarchy, Python has a concept of #link("https://docs.python.org/3/glossary.html#term-package")[packages].
]
...
#quote(block: true)[
  A package’s `__init__.py` file may set or alter the package’s `__path__` attribute, and this was typically the way namespace packages were implemented prior to #link("https://peps.python.org/pep-0420")[PEP 420].
...
#quote(block: true)[
  ...
  A third default finder searches an #link("https://docs.python.org/3/glossary.html#term-import-path")[import path] for modules.
...
#link("https://docs.python.org/3/library/sys.html#sys.path")[sys.path] contains a list of strings providing search locations for modules and packages.
...
- #link("https://docs.python.org/3/using/cmdline.html#cmdoption-m")[`python -m module`] command line: prepend the current working directory.
...
=== 1.3 #link("https://docs.python.org/3/reference/import.html#package-relative-imports")[相对导入]
...
#link("https://docs.python.org/3/tutorial/modules.html#intra-package-references")[Intra-package References]:
