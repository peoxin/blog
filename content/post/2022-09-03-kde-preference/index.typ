...
#show: post.with(
  ...
  pub-date: datetime(year: 2022, month: 9, day: 3),
  mod-date: datetime(year: 2022, month: 9, day: 3),
)
...
Layout 文件：#link("https://github.com/peoxin/dotfiles")[peoxin/dotfiles]
...
全局选项 > 向前切换输入法：`Alt` + `左Shift`

全局选项 > 向后切换输入法：`Alt` + `右Shift`
...
高级 > CapsLock 行为：将 `CapsLock` 作为额外的 `Esc`，`Shift` + `CapsLock` 作为 `Compose`
